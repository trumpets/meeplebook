...
- Global play-timer infrastructure lives under `core/timer/service/`: `PlayTimerServiceCoordinator` is injected from `MeepleBookApp` to mirror persisted timer state into the service lifecycle, while the foreground service, action receiver, and boot receiver stay thin and delegate to timer use cases/repository state. Keep the service declared as `foregroundServiceType="specialUse"` with `android.app.PROPERTY_SPECIAL_USE_FGS_SUBTYPE`, and guard notification updates with the Android 13 notification-permission check plus the narrow lint suppression already used in `PlayTimerForegroundService`.
