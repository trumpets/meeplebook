...
MeepleBook uses typed `@Serializable` routes, but currently resolves navigation arguments in the
`NavHost` (`backStackEntry.toRoute<T>()`) and passes plain arguments into screen composables. Do not
assume that ViewModels read routes from `SavedStateHandle` unless the screen is already built that
way or the user asks to move in that direction.
...
```kotlin
// build.gradle.kts
androidTestImplementation("<use the repo's current navigation-testing artifact/version>")
```
...
- In MeepleBook, prefer decoding routes in the NavHost and passing plain args into screens; use `SavedStateHandle.toRoute<T>()` in ViewModels only when that screen is already designed around it
...
> MeepleBook-specific preference: decode routes in the navigation layer first, then pass the small
> values the screen actually needs. This is the current repo pattern for screens like Add Play.
