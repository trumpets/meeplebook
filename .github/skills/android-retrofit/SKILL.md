...
MeepleBook uses Retrofit primarily as a **transport layer** for BGG XML endpoints:

- `BggApi` returns `Response<ResponseBody>`
- XML is parsed manually with XmlPull parsers
- remote data sources explicitly handle `202`, `429`, and `5xx`
- retry/backoff is centralized instead of hidden behind a generic converter stack

So when applying this skill here, prefer raw-body + manual parse guidance over JSON-first examples.
...
> MeepleBook-specific preference: use `Response<ResponseBody>` when the endpoint returns XML and the
> code needs explicit status handling before parsing.
