...
MeepleBook is not modularized into `:feature:*` Gradle modules yet. Use existing module/task names
from the repo (`:app`, `:lint-rules`, `testDebugUnitTest`, `connectedDebugAndroidTest`, `lint`)
instead of generic affected-module examples that assume a multi-module setup.
...
# Only run repo-valid targeted tasks
