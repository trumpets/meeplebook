...
Use the **current repository structure as the source of truth**. MeepleBook is currently a
single-module app (`:app`) plus `:lint-rules`, with `core/`, `feature/`, and `ui/` organized as
packages inside `app/src/main/java/app/meeplebook/`.

Multi-module architecture is still the intended **future direction**, but it must be treated as a
later-stage migration target, not as something that already exists. Do not prescribe `:core:*` or
`:feature:*` Gradle modules unless the user is explicitly asking for modularization work.
...
For production apps, use a multi-module strategy to improve build times and separation of
concerns.

> MeepleBook-specific rule: keep the current package-based single-module structure unless the task
> is specifically about modularization. When giving repo-specific advice, describe the current
> `app/.../core`, `app/.../feature`, and `app/.../ui` package layout first, then optionally note
> the future multi-module destination.
